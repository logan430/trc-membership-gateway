...
must_haves:
  truths:
    ...
    - "Squire role has restricted channel access (only #introductions and onboarding)"
  artifacts:
...
<task type="checkpoint:human-action" gate="blocking">
  <name>Task 3: Configure Squire Role Channel Permissions</name>
  <action>
Configure the Discord server so that the Squire (Paid Unintroduced) role can ONLY access #introductions and onboarding channels.

This is a one-time Discord server configuration that must be done manually in Discord's server settings.
  </action>
  <instructions>
**In Discord Server Settings:**

1. **Go to Server Settings > Roles**
   - Find or verify the "Squire" role exists (created by bot in Phase 2)
   - Note: Bot creates roles automatically, but channel permissions are server-level config

2. **Configure @everyone role (base permissions)**
   - Server Settings > Roles > @everyone
   - Under "Text Channel Permissions", DENY:
     - View Channels (this hides all channels by default)
   - Save changes

3. **Configure #introductions channel**
   - Right-click #introductions > Edit Channel > Permissions
   - Click "+" to add a role, select "Squire"
   - Set these permissions for Squire:
     - View Channel: ALLOW
     - Send Messages: ALLOW
     - Read Message History: ALLOW
     - Add Reactions: ALLOW (optional)
   - Save changes

4. **Configure onboarding channels** (any welcome/rules channels)
   - For each onboarding channel, repeat step 3:
     - Add Squire role
     - Allow View Channel and Read Message History
     - (Send Messages only if they should be able to post)

5. **Verify Knight/Lord roles have full access**
   - These roles (assigned after introduction) should have View Channel ALLOW on all member channels
   - Configure on each channel or use a category-level permission

**Verification:**
- Create a test account, complete payment and claim flow
- Confirm the test account with Squire role can ONLY see #introductions and onboarding channels
- Confirm they cannot see general member channels
  </instructions>
  <resume-signal>Type "configured" after completing Discord server channel permissions setup, or describe any issues encountered</resume-signal>
</task>
...
5. Discord server configured: Squire role only sees #introductions and onboarding channels
</verification>
...
- Squire role restricted to #introductions and onboarding channels (ONB-05)
</success_criteria>
