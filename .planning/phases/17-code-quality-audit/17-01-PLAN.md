...
must_haves:
  truths:
    ...
    - "Error handling patterns are analyzed for consistency"
  ...
  key_links:
    ...
    - from: "dpdm output"
      ...
      via: "tool output analysis"
    - from: "try/catch grep output"
      to: "CODE-QUALITY-AUDIT.md error handling section"
...
Use Grep tool to search for error handling patterns:

- Pattern: `try\s*\{`
- Path: src/
- Output mode: content with context (-A 5 to see the catch block)

- Pattern: `catch\s*\(`
- Path: src/
- Output mode: content with context (-B 2 -A 3 to see error handling)

- Pattern: `\.catch\(`
- Path: src/
- Output mode: content (for Promise catch handlers)

- Pattern: `throw new`
- Path: src/
- Output mode: content (to see what errors are thrown)

- Pattern: `res\.(status|json).*error`
- Path: src/
- Output mode: content (for API error responses)

Analyze patterns for consistency:
- Are catch blocks logging errors consistently (using Pino logger vs console)?
- Are API routes returning consistent error response shapes (e.g., {error: string})?
- Are errors being swallowed (empty catch blocks)?
- Is there a consistent pattern for error types (custom Error classes vs generic Error)?

Record all tool outputs for the next task.
  </action>
  <verify>
All 7 audit categories have been executed:
...
- Error handling pattern grep completed
  </verify>
  <done>Tool outputs for all 7 audit categories are available for report generation</done>
</task>
...
[Analysis of error handling consistency across the codebase]

**Pattern Inventory:**
- Total try/catch blocks: N
- Promise .catch() handlers: N
- Custom error classes: [list or "none"]
- Error response patterns in API routes: [describe consistency]

**Consistency Assessment:**
- [ ] Catch blocks use consistent logging (Pino vs console)
- [ ] API routes return consistent error shapes
- [ ] No empty catch blocks (errors swallowed)
- [ ] Error messages are user-friendly (no stack traces exposed)

**Issues Found:**
| Location | Issue | Severity |
|----------|-------|----------|
| [file:line] | [inconsistency description] | Medium/High |
...
- **High:** Circular dependencies, dead exports in public API, high npm vulnerabilities, inconsistent error handling in auth/payment paths
- **Medium:** Unused files, implicit `any` types, medium npm vulnerabilities, empty catch blocks, inconsistent error response shapes
- **Low:** Unused devDependencies, informational console.log, low npm vulnerabilities, minor error handling inconsistencies
...
- For error handling: flag empty catch blocks and inconsistent error response formats as issues
  </action>
  <verify>
```bash
...
- All 7 audit categories documented (including error handling patterns)
...
2. Report covers all 7 audit categories from ROADMAP success criteria
3. Each finding has severity classification
4. Error handling patterns analyzed for consistency
5. Recommendations are actionable for future cleanup phase
6. No code changes were made (audit only)
</verification>
...
- Error handling patterns analyzed and consistency assessed
