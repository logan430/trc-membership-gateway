...
must_haves:
  truths:
    ...
    - "Admin can list ALL resources including drafts, scheduled, and deleted"
  ...
  artifacts:
    - path: "src/resources/service.ts"
      ...
      exports: ["createResource", "updateResource", "uploadNewVersion", "softDeleteResource", "listResources", "listResourcesAdmin", "downloadResource", "getResourceAnalytics"]
...
grep -q "listResourcesAdmin" src/resources/service.ts
  </verify>
  <done>
Resource service complete with:
- Admin CRUD (create, update metadata, soft delete)
- Admin listing (listResourcesAdmin - includes drafts, scheduled, deleted)
...
- [ ] listResourcesAdmin returns all statuses including drafts/scheduled/deleted
...
4. Admin listing: ALL resources shown (drafts, scheduled, deleted if requested)
5. Download flow: signed URL + ResourceDownload record + points award
6. Points: first download only per member+resource (idempotency from points service)
7. Analytics: totals, per-resource stats, trending (7-day window)
8. Recommendations: tag-based matching from download history
</success_criteria>
