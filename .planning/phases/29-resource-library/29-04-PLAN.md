...
must_haves:
  truths:
    ...
    - "Admin can list ALL resources (drafts, scheduled, published, deleted) via GET /api/admin/resources"
  ...
  key_links:
    - from: "src/routes/admin/resources.ts"
      ...
      pattern: "createResource\\(|updateResource\\(|uploadNewVersion\\(|listResourcesAdmin\\("
    - from: "src/routes/admin/resources.ts"
...
Purpose: Expose all resource functionality via REST API endpoints. Admin routes handle file uploads with rate limiting and validation. Admin list endpoint uses listResourcesAdmin() to show ALL resources (drafts, scheduled, deleted). Member routes provide read-only access to published resources.
...
grep -q "listResourcesAdmin" src/routes/admin/resources.ts
  </verify>
  <done>
Admin resource routes complete:
- POST / - Upload new resource (rate limited)
- GET / - List ALL resources (drafts, scheduled, published, deleted) using listResourcesAdmin()
...
- [ ] Admin GET / uses listResourcesAdmin() to return ALL statuses (drafts, scheduled, published, deleted)
...
2. Admin can list ALL resources (drafts, scheduled, published, deleted) via GET /api/admin/resources
3. Admin can update metadata via PATCH /api/admin/resources/:id
4. Admin can upload new versions via POST /api/admin/resources/:id/version
5. Admin can soft delete via DELETE /api/admin/resources/:id
6. Admin can view analytics via GET /api/admin/resources/analytics
7. Admin can manage tags via /api/admin/resources/tags endpoints
8. Member can browse resources via GET /api/resources with filters
9. Member can view details via GET /api/resources/:id
10. Member can download via POST /api/resources/:id/download (returns signed URL)
11. Member can get recommendations via GET /api/resources/recommended
12. All endpoints properly authenticated and validated
</success_criteria>
