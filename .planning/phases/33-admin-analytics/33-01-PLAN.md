...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/analytics/benchmark-analytics.ts"
      ...
      exports: ["getBenchmarkStats", "getBenchmarkTrends"]
...
import type { BenchmarkStats, CategoryStats, BenchmarkTrend, DateRange, SegmentFilters } from './types.js';
...
// ANALYTICS-05: Industry insights by segment (aggregate patterns)
export async function getBenchmarkInsights(filters: SegmentFilters): Promise<Record<string, unknown>> {
  // Build JSONB path filter if companySize provided (from BUSINESS category data)
  // This is a simplified version - full implementation would need more complex JSONB queries
  ...
  // Calculate aggregate patterns from JSONB data
  // Returns summary statistics for key business metrics
  return {
    submissionCount: submissions.length,
    filters,
    // Actual aggregation would parse JSONB and calculate medians/averages
  };
}
```
...
<success_criteria>
- [ ] src/analytics/types.ts exists with all analytics interfaces
...
- [ ] src/analytics/benchmark-analytics.ts exports getBenchmarkStats, getBenchmarkTrends
