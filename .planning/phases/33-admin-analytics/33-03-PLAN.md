...
Note: SegmentFilters component was removed from this plan - segment filtering will be handled inline in the page or deferred to a future enhancement. The current dashboard focuses on the core analytics views without segment-based filtering UI.
  </action>
  <verify>TypeScript compiles: `cd dashboard && npx tsc --noEmit`</verify>
...
- [ ] dashboard/src/components/admin/ has all 6 components (KpiCard, TimeSeriesChart, ComparisonBarChart, RetentionHeatmap, AtRiskMemberList, DateRangePicker)
