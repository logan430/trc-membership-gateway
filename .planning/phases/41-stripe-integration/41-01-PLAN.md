...
must_haves:
  truths:
    ...
    - "App restarts successfully with Stripe env vars (no Zod validation crash)"
  artifacts:
...
<task type="auto">
  ...
  6. Verify the app started without Zod validation errors by checking health:
     ```bash
...
- No Zod validation errors in app logs
</verification>
